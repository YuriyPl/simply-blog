...
import com.github.ypl.simplyblog.service.email.EmailSender;
import com.github.ypl.simplyblog.util.EmailValidator;
...
public class RegistrationService {
    ...
    @Value("${ypl.app.confirmTokenLink}")
    ...
    public User register(RegistrationRequest request) {
        ...
        User user = new User(request.getName(), request.getEmail(), request.getPassword(), false, Role.USER);

        String token = userDetailsService.signUpUser(user);
        String link = confirmTokenLink + token;
    ...
    public void confirmToken(String token) {
        ConfirmationToken confirmationToken = confirmationTokenRepository.findByToken(token)
                .orElseThrow(() ->
                        new IllegalRequestDataException(token + " token not found"));

        checkNotConfirmed(confirmationToken);
        checkNotExpired(confirmationToken);
    ...
    private void checkNotConfirmed(ConfirmationToken confirmationToken) {
        if (confirmationToken.getConfirmedAt() != null) {
            throw new IllegalRequestDataException(confirmationToken.getUser().getEmail() + " email already confirmed");
        }
    }

    private void checkNotExpired(ConfirmationToken confirmationToken) {
        if (confirmationToken.getExpiresAt().isBefore(Instant.now())) {
            throw new IllegalRequestDataException(confirmationToken + " token expired");
        }
    }
